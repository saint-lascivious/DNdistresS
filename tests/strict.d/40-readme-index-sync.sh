...
# readme-index-sync.sh - checks README Index anchors vs CLI --help topics.
...
for t in $index_topics; do
    ...
    if ! printf '%s\n' "$cli_topics" | grep -Fx -- "$t" >/dev/null 2>&1; then
        ...
        not_ok "README index anchor maps to CLI topic: $t"

    else

        ok "README index anchor maps to CLI topic: $t"
