...

{
    ...
    cat <<EOF
<!--
  Generated by $SCRIPT_NAME for $DOC_NAME $DOC_VERSION at $GENERATED_AT.
-->
...
printf '\n---\n\n> Generated by [%s](scripts/%s) for [%s](%s) %s at %s.\n' \
    "$(basename -- "$0")" "$(basename -- "$0")" "$DOC_NAME" "$DOC_NAME" \
    "$DOC_VERSION" "$GENERATED_AT"
} > "$TMP_OUT" || {
